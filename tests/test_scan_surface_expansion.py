...
from agents.cli import AGENTS, RULES, _project_runtime_commands, _run_runtime_verification
...
def test_new_agents_are_registered():
    assert {"flow_audit", "frontend_performance", "iac_security", "supply_chain_audit"} <= set(AGENTS)


def test_config_audit_new_surfaces_are_wired():
    globs = {(glob, tool) for glob, _, agent, tool, _ in RULES if agent == "config_audit"}
...
def test_api_graphql_and_rate_limit_contract_detection():
    ...
    gql = agent._review_graphql_error_contract("return { errors: [{ message: 'bad' }] }")
...
def test_runtime_verification_runs_multiple_detected_commands(tmp_path, monkeypatch):
    (tmp_path / "package.json").write_text(json.dumps({"scripts": {"test": "echo ok", "build": "echo ok", "lint": "echo ok"}}))
