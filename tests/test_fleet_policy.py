...
def test_trivy_split_gate_is_clean():
    assert (
        check_trivy_sarif_gate(".github/workflows/security-scan.yml", TRIVY_FIXED) == []
    )
...
def test_direct_dependency_holdback_without_ignore_is_flagged():
    ...
    out = check_forced_version_without_dependabot_ignore(
        pkg, "version: 2\nupdates: []\n"
    )
...
def test_transitive_only_override_is_not_flagged():
    ...
    pkg = json.dumps(
        {"dependencies": {"react": "^18.2.0"}, "overrides": {"postcss": "8.5.26"}}
    )
...
def test_forced_version_with_matching_ignore_is_clean():
    pkg = json.dumps(
        {"devDependencies": {"postcss": "^8.5.26"}, "overrides": {"postcss": "8.5.26"}}
    )
...
def test_wildcard_ignore_matches_prefix():
    pkg = json.dumps(
        {
            "dependencies": {"@radix-ui/react-slot": "^1.3.3"},
...
def test_sarif_with_limit_severities_is_not_flagged():
    ...
    assert (
        check_trivy_sarif_gate(
            ".github/workflows/security-scan.yml", TRIVY_SARIF_BUT_LIMITED
        )
        == []
    )
