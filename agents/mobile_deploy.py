...
"""
        ...
        return [
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "eas_json": {
                            "type": "string",
                            "description": "The contents of eas.json",
                        },
                    },
                    "required": ["eas_json"],
                },
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "codemagic_yaml": {
                            "type": "string",
                            "description": "The contents of codemagic.yaml",
                        },
                    },
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "has_iap": {
                            "type": "boolean",
                            "description": "Whether the app has in-app purchases/subscriptions",
                        },
                        "uses_healthkit": {"type": "boolean"},
                        "uses_tracking": {
                            "type": "boolean",
                            "description": "Whether the app does any IDFA-adjacent tracking (requires ATT prompt)",
                        },
                    },
                    "required": ["platform"],
                },
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The RevenueCat integration code to review",
                        },
                    },
                    "required": ["code"],
                },
    ...
    def _review_eas_config(self, eas_json: str) -> Dict[str, Any]:
        ...
        secret_key_pattern = re.compile(
            r"(secret|token|api[_-]?key|password|credential)", re.IGNORECASE
        )
        ...
        for profile_name, profile in profiles.items():
            ...
            for key, value in env.items():
                public_bundle_value = key.upper().startswith(
                    "EXPO_PUBLIC_"
                ) and not re.search(
                    r"SECRET|PRIVATE|ADMIN|AUTH_TOKEN", key, re.IGNORECASE
                )
                if (
                    secret_key_pattern.search(key)
                    and not public_bundle_value
                    and isinstance(value, str)
                    and value
                    and not reference_pattern.match(value)
                ):
        ...
        if "production" not in profiles:
            findings.append(
                {
                    "severity": "HIGH",
                    "setting": "build.production",
                    "issue": "No 'production' build profile found",
                    "fix": "Add a production profile with credentialsSource and autoIncrement set",
                }
            )
        else:
            ...
            if not prod.get("autoIncrement"):
                findings.append(
                    {
                        "severity": "LOW",
                        "setting": "build.production.autoIncrement",
                        "issue": "autoIncrement not set on the production profile — build numbers must be bumped manually",
                        "fix": 'Set "autoIncrement": true on the production profile',
                    }
                )
            ...
            findings.append(
                {
                    "severity": "INFO",
                    "setting": "submit",
                    "issue": "No 'submit' section — eas submit will require inline flags every time",
                    "fix": "Add a submit.production block (App Store Connect / Play Console identifiers)",
                }
            )
    ...
    def _review_codemagic_config(self, codemagic_yaml: str) -> Dict[str, Any]:
        ...
        if re.search(r"-----BEGIN (RSA )?PRIVATE KEY-----", codemagic_yaml):
            findings.append(
                {
                    "severity": "CRITICAL",
                    "issue": "A private key appears to be inlined directly in codemagic.yaml",
                    "fix": "Store signing keys as encrypted Codemagic environment variables or use the code_signing_identities integration — never commit key material",
                }
            )

        has_ios_workflow = bool(re.search(r"\bios\b", codemagic_yaml, re.IGNORECASE))
        has_signing_config = bool(
            re.search(
        ...
        if has_ios_workflow and not has_signing_config:
            findings.append(
                {
                    "severity": "INFO",
                    "issue": "No explicit code-signing configuration found for what looks like an iOS workflow — confirm signing is handled via App Store Connect API integration or an explicit certificate/profile setup",
                    "fix": "Add an ios_signing or code_signing_identities block, or confirm automatic signing via an App Store Connect integration",
                }
            )
        ...
        if not re.search(r"triggering:|events:", codemagic_yaml):
            findings.append(
                {
                    "severity": "LOW",
                    "issue": "No explicit triggering config found — confirm this workflow isn't relying on manual-only runs when you expect it to be automatic (or vice versa)",
                    "fix": "Add a triggering.events block (push/tag/pull_request) if this should run automatically",
                }
            )

        if re.search(
            r"testflight|app_store_connect", codemagic_yaml, re.IGNORECASE
        ) and not re.search(
            r"submit_to_testflight|submit_to_app_store|beta_groups",
            codemagic_yaml,
            re.IGNORECASE,
        ):
            findings.append(
                {
                    "severity": "INFO",
                    "issue": "App Store Connect integration referenced but no explicit submit_to_testflight/submit_to_app_store step found",
                    "fix": "Add publishing.app_store_connect.submit_to_testflight (or submit_to_app_store) so the build is actually delivered, not just archived",
                }
            )

        workflow_count = len(
            re.findall(r"^\s{2}[\w-]+:\s*$", codemagic_yaml, re.MULTILINE)
        )

        return {
            "workflow_blocks_detected": workflow_count,
            "findings": findings,
            "total_issues": len(findings),
        }
    ...
    def _app_store_submission_checklist(
        ...
        checklist = [
            {
                "item": "App Store Connect / Play Console metadata complete (description, keywords, support URL)",
                "critical": True,
            },
            {
                "item": "Screenshots provided for every required device size",
                "critical": True,
            },
            {"item": "Privacy policy URL set and reachable", "critical": True},
            {
                "item": "Privacy nutrition label / Data Safety form filled out accurately (matches what the app actually collects)",
                "critical": True,
            },
        ...
        if platform in ("ios", "both"):
            checklist.append(
                {
                    "item": "Export compliance (ITSAppUsesNonExemptEncryption) set correctly in Info.plist/eas.json",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "TestFlight internal build validated before submitting for full App Review",
                    "critical": False,
                }
            )
        ...
        if platform in ("android", "both"):
            checklist.append(
                {
                    "item": "Play Console Data Safety section matches SDKs actually bundled (RevenueCat, Sentry, analytics)",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "Target API level meets Play Store's current minimum requirement",
                    "critical": True,
                }
            )
        ...
        if has_iap:
            checklist.append(
                {
                    "item": "All IAP products/subscriptions are in 'Ready to Submit' state in App Store Connect / Play Console before referencing them in a submitted build",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "Restore Purchases is reachable from the UI (App Review requirement)",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "Subscription terms (price, duration, auto-renewal) disclosed in-app before purchase",
                    "critical": True,
                }
            )
        ...
        if uses_healthkit:
            checklist.append(
                {
                    "item": "HealthKit usage strings (NSHealthShareUsageDescription / NSHealthUpdateUsageDescription) are specific about what's read/written",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "Health data is not used for advertising — confirm this explicitly, App Review checks it",
                    "critical": True,
                }
            )
        ...
        if uses_tracking:
            checklist.append(
                {
                    "item": "App Tracking Transparency (ATT) prompt shown before any IDFA-adjacent tracking begins",
                    "critical": True,
                }
            )
            checklist.append(
                {
                    "item": "NSUserTrackingUsageDescription set in Info.plist",
                    "critical": True,
                }
            )
        ...
        if app_category == "finance_security":
            checklist.append(
                {
                    "item": "Extra App Review scrutiny expected for finance/security categories — budget review time accordingly, and have documentation ready for reviewer questions",
                    "critical": False,
                }
            )
    ...
    def _review_revenuecat_setup(self, code: str) -> Dict[str, Any]:
        ...
        if (
            "purchases.configure" not in code_lower
            and "purchases.configure(" not in code_lower.replace(" ", "")
        ):
            findings.append(
                {
                    "severity": "HIGH",
                    "issue": "No Purchases.configure() call found — RevenueCat must be configured once, early, before any paywall can render",
                    "fix": "Call Purchases.configure({ apiKey }) at app startup, before rendering any screen that checks entitlements",
                }
            )

        if "getofferings" in code_lower or "offerings" in code_lower:
            if not re.search(
                r"catch|error|loading|isloading|try\s*{", code, re.IGNORECASE
            ):
                findings.append(
                    {
                        "severity": "MEDIUM",
                        "issue": "Offerings fetched without a visible loading/error state — offerings can be empty or fail to load (network, misconfigured products)",
                        "fix": "Handle the empty/error offerings case explicitly rather than assuming a paywall always has products to show",
                    }
                )
        ...
        if not re.search(r"restorepurchases", code_lower):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No restorePurchases() call found — Apple requires a visible restore option for any app with IAP",
                    "fix": "Expose a 'Restore Purchases' action that calls Purchases.restorePurchases()",
                }
            )

        if re.search(r"purchasepackage|purchaseproduct", code_lower) and not re.search(
            r"entitlements", code_lower
        ):
            findings.append(
                {
                    "severity": "HIGH",
                    "issue": "Purchase flow found but no entitlements check — gate premium features on customerInfo.entitlements.active, not on the raw purchase call succeeding",
                    "fix": "After purchase, check customerInfo.entitlements.active['<entitlement_id>'] rather than assuming success == entitled",
                }
            )
