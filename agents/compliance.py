...
_FRAMEWORKS: Dict[str, Dict[str, Dict[str, Any]]] = {
    "SOC2": {
        "CC6.1": {
            ...
            "checks": [
                {"pattern": r"jwt\.(sign|verify)\(", "negate": False, "label": "JWT auth present"},
                {"pattern": r"oauth|AppleAuthentication|google.*oauth", "negate": False, "label": "OAuth integration present"},
                {"pattern": r"x-api-key|requireApiKey|INTERNAL_API_KEY", "negate": False, "label": "API key gate present"},
            ],
        ...
        "CC6.2": {
            ...
            "checks": [
                {"pattern": r"https://|ssl:|tls:", "negate": False, "label": "TLS/HTTPS references"},
                {"pattern": r"bcrypt|argon2|hashlib\.pbkdf2|sha256", "negate": False, "label": "Password hashing present"},
                {"pattern": r"(?i)http://(?!localhost|127\.0\.0\.1)", "negate": True, "label": "No plaintext HTTP to remote hosts"},
            ],
        ...
        "CC6.7": {
            ...
            "checks": [
                {"pattern": r"stripe\.webhooks\.constructEvent|svix.*verify", "negate": False, "label": "Webhook signature verification"},
                {"pattern": r"hsts|Strict-Transport-Security", "negate": False, "label": "HSTS header present"},
            ],
        ...
        "CC7.2": {
            ...
            "checks": [
                {"pattern": r"sentry_sdk\.init\(|Sentry\.init\(", "negate": False, "label": "Sentry error monitoring"},
                {"pattern": r"sendDefaultPii\s*:\s*false", "negate": False, "label": "Sentry PII scrubbing configured"},
                {"pattern": r"/health|/ping|/status", "negate": False, "label": "Health endpoint present"},
            ],
        ...
        "CC8.1": {
            ...
            "checks": [
                {"pattern": r"\.github/workflows|ci\.yml|ci\.yaml", "negate": False, "label": "CI pipeline present"},
                {"pattern": r"test\s*:|scripts.*test", "negate": False, "label": "Test scripts defined"},
            ],
    ...
    "HIPAA": {
        "164.312(a)(1)": {
            ...
            "checks": [
                {"pattern": r"jwt\.(sign|verify)\(|Bearer\b", "negate": False, "label": "User authentication tokens"},
                {"pattern": r"exp\s*:|exp:\s*\d|expiresIn\s*:", "negate": False, "label": "Token expiry set"},
                {"pattern": r"logout|signOut|revoke", "negate": False, "label": "Session termination"},
            ],
        ...
        "164.312(b)": {
            ...
            "checks": [
                {"pattern": r"audit_log|auditLog|logger\.(info|warn|error)", "negate": False, "label": "Audit logging present"},
                {"pattern": r"winston|pino|structlog", "negate": False, "label": "Structured logging library"},
            ],
        ...
        "164.312(e)(2)(ii)": {
            ...
            "checks": [
                {"pattern": r"https://|ssl:|tls:", "negate": False, "label": "HTTPS/TLS references"},
                {"pattern": r"(?i)http://(?!localhost|127\.0\.0\.1)", "negate": True, "label": "No HTTP to remote endpoints"},
            ],
    ...
    "GDPR": {
        "Art.25": {
            ...
            "checks": [
                {"pattern": r"sensitive_data|@sensitive|pii|personally.identifiable", "negate": False, "label": "PII tagging present"},
                {"pattern": r"sendDefaultPii\s*:\s*false|beforeSend", "negate": False, "label": "PII scrubbing in error reporting"},
            ],
        ...
        "Art.32": {
            ...
            "checks": [
                {"pattern": r"bcrypt|argon2|sha256", "negate": False, "label": "Password/data hashing"},
                {"pattern": r"jwt\.(sign|verify)\(|oauth", "negate": False, "label": "Authentication present"},
                {"pattern": r"https://|tls:", "negate": False, "label": "Transport encryption"},
            ],
        ...
        "Art.33": {
            ...
            "checks": [
                {"pattern": r"sentry_sdk\.init\(|Sentry\.init\(|pagerduty|opsgenie|alertmanager", "negate": False, "label": "Incident monitoring present"},
                {"pattern": r"audit_log|access_log|security.log", "negate": False, "label": "Audit trail logging"},
            ],
    ...
    "PCI-DSS": {
        "Req.2": {
            ...
            "checks": [
                {"pattern": r"(?i)password\s*[:=]\s*['\"](?:admin|password|1234|default|changeme)", "negate": True, "label": "No default passwords"},
                {"pattern": r"helmet\(|cors\(", "negate": False, "label": "Security headers configured"},
            ],
        ...
        "Req.6": {
            ...
            "checks": [
                {"pattern": r"npm\s+audit|safety\s+check|trivy|snyk", "negate": False, "label": "Dependency vulnerability scanning"},
                {"pattern": r"z\.object\(|Joi\.object\(|validate\(|sanitize\(", "negate": False, "label": "Input validation present"},
            ],
        ...
        "Req.8": {
            ...
            "checks": [
                {"pattern": r"jwt\.(sign|verify)\(|passport|OAuth2", "negate": False, "label": "Authentication library"},
                {"pattern": r"rate.?limit|throttle|rateLimit", "negate": False, "label": "Brute-force protection"},
                {"pattern": r"bcrypt|argon2", "negate": False, "label": "Password hashing present"},
            ],
        ...
        return [
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "code": {"type": "string", "description": "Source code or config to audit"},
                        "standard": {
                            "type": "string",
