...
class ConfigAuditAgent(BaseAgent):
    ...
    def _define_tools(self) -> List[Dict[str, Any]]:
        def tool(name: str, desc: str, extra: Dict[str, Any] | None = None) -> Dict[str, Any]:
            props: Dict[str, Any] = {
                "content": {"type": "string", "description": "The file's full text"},
                "path": {"type": "string", "description": "Path to the file, used for context"},
            }
            props.update(extra or {})
            return {
                "name": name,
                "description": desc,
                "parameters": {"type": "object", "properties": props, "required": ["content"]},
            }

        return [
            tool("audit_dockerfile", "Audit a Dockerfile for root execution, unpinned bases, baked-in secrets, and curl|sh installs."),
            tool("audit_compose", "Audit a docker-compose file for privileged containers, published database ports, and committed credentials."),
            tool("audit_workflow", "Audit a GitHub Actions workflow for unpinned actions, missing permissions, pull_request_target, and script injection."),
            tool("audit_android_manifest", "Audit AndroidManifest.xml for cleartext traffic, debuggable builds, backup exposure, and exported components without a permission."),
            tool("audit_ios_plist", "Audit an iOS Info.plist for App Transport Security exceptions."),
    ...
    def _audit_dockerfile(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        stage_starts = [i for i, l in enumerate(lines) if re.match(r"^\s*FROM\b", l, re.I)]
        stage_aliases = {
            m.group(1) for l in lines
            for m in [re.match(r"^\s*FROM\s+\S+\s+AS\s+(\S+)", l, re.I)] if m
        }
        if not stage_starts:
            return {"findings": findings, "total_issues": 0}
        final = lines[stage_starts[-1]:]
        if not any(re.match(r"^\s*USER\s+(?!root\b)\S+", l, re.I) for l in final):
            findings.append(_finding(
                "HIGH", "Final stage has no USER instruction — the container runs as root",
                stage_starts[-1] + 1,
        ...
        for i, l in enumerate(lines):
            ...
            if m:
                ...
                if ":" not in ref.split("/")[-1] or ref.endswith(":latest"):
                    findings.append(_finding(
                        "MEDIUM", f"Base image `{ref}` is unpinned — a rebuild can silently pull a different image",
                        i + 1, "Pin at least a minor version (`node:20.19-alpine`), or a digest for reproducible builds.",
                    ))
            if re.match(r"^\s*(ENV|ARG)\s+", l, re.I):
                for name, value in re.findall(r"([A-Za-z_][A-Za-z0-9_]*)\s*=\s*\"?([^\s\"]+)", l):
                    if _SECRET_NAME.search(name) and value and not _PLACEHOLDER.search(value):
                        findings.append(_finding(
                            "CRITICAL", f"`{name}` is baked into the image via {l.split()[0].upper()} — it ships in every layer and every registry copy",
                            i + 1, "Pass secrets at runtime (Railway variables, `docker run -e`), never at build time. Use `--mount=type=secret` if a build genuinely needs one.",
                        ))
            if re.search(r"\b(curl|wget)\b[^|\n]*\|\s*(ba)?sh\b", l):
                findings.append(_finding(
                    "HIGH", "Pipes a download straight into a shell — the build trusts whatever that URL serves today",
                    i + 1, "Download to a file, verify a checksum, then execute.",
                ))
            if re.match(r"^\s*ADD\s+https?://", l, re.I):
                findings.append(_finding(
                    "MEDIUM", "`ADD` with a URL fetches at build time with no integrity check",
                    i + 1, "Use `RUN curl -fsSL ... -o file && sha256sum -c` instead.",
                ))
    ...
    def _audit_compose(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        for m in re.finditer(r"^\s*privileged:\s*true", content, re.M):
            findings.append(_finding(
                "HIGH", "A service runs `privileged: true` — full host device access",
                _line_of(content, m.start()), "Drop it; grant specific `cap_add` entries if one is genuinely needed.",
            ))
        for m in re.finditer(r"^\s*network_mode:\s*[\"']?host", content, re.M):
            findings.append(_finding(
                "MEDIUM", "`network_mode: host` — the container shares the host's network namespace",
                _line_of(content, m.start()), "Use the default bridge network and publish only the ports you need.",
            ))
        ...
        for m in re.finditer(r"^\s*-\s*[\"']?(?:0\.0\.0\.0:)?(5432|3306|27017|6379):\1[\"']?\s*$", content, re.M):
            findings.append(_finding(
                "LOW", f"Database port {m.group(1)} is published to all host interfaces",
                _line_of(content, m.start()), f"Bind to loopback: `\"127.0.0.1:{m.group(1)}:{m.group(1)}\"` — same convenience, not reachable from the LAN.",
            ))
        for m in re.finditer(r"^\s*(POSTGRES_PASSWORD|MYSQL_ROOT_PASSWORD|MYSQL_PASSWORD|REDIS_PASSWORD|MONGO_INITDB_ROOT_PASSWORD)\s*[:=]\s*[\"']?([^\s\"'$]+)", content, re.M):
            findings.append(_finding(
                "LOW", f"`{m.group(1)}` is a literal in the compose file (dev credential committed)",
                _line_of(content, m.start()), "Fine for a local harness, but reference `${POSTGRES_PASSWORD:-dev}` so the same file cannot be reused with a real value by accident.",
            ))
        return {"findings": findings, "total_issues": len(findings)}
    ...
    def _audit_workflow(self, content: str, path: str = "") -> Dict[str, Any]:
        findings: List[Finding] = []
        if path and "/.github/workflows/" not in path.replace("\\", "/") and not path.startswith(".github/workflows/"):
            return {"findings": findings, "total_issues": 0}

        if re.search(r"^\s*pull_request_target\s*:", content, re.M) and re.search(r"actions/checkout@", content):
            findings.append(_finding(
                "CRITICAL", "`pull_request_target` with a checkout — a fork PR can run its code with this repo's secrets",
                None, "Use `pull_request` unless you need the base repo's secrets, and never check out the PR head under `pull_request_target`.",
            ))

        # Expression injection: untrusted event fields interpolated straight
        # into a shell step.
        for m in re.finditer(r"run:\s*[|>]?[^\n]*\$\{\{\s*github\.event\.(?:issue|pull_request|comment|head_commit|review)\.(?:title|body|message)", content):
            findings.append(_finding(
                "HIGH", "Untrusted event text is interpolated into a `run:` step — a crafted PR title runs as shell",
                _line_of(content, m.start()), "Pass it through an env var (`env: TITLE: ${{ ... }}`) and reference `\"$TITLE\"` in the script.",
            ))
        ...
        if not re.search(r"^\s*permissions\s*:", content, re.M):
            findings.append(_finding(
                "MEDIUM", "No `permissions:` block anywhere — the GITHUB_TOKEN gets the repository default, which is often write",
                None, "Add `permissions: { contents: read }` at the top (or per job) and widen only where needed.",
            ))
        if re.search(r"^\s*permissions\s*:\s*write-all", content, re.M):
            findings.append(_finding(
                "HIGH", "`permissions: write-all` — every scope, for every step",
                None, "List the scopes each job actually uses.",
            ))
        ...
        for m in re.finditer(r"^\s*-?\s*uses:\s*([\w.-]+/[\w./-]+)@([^\s#]+)", content, re.M):
            ...
            findings.append(_finding(
                ...
                + ("First-party action, lower risk." if first_party else "Third-party: a compromised tag runs with this workflow's permissions."),
            ))
        return {"findings": findings, "total_issues": len(findings)}
    ...
    def _audit_android_manifest(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        m = None if is_debug_variant else re.search(r'usesCleartextTraffic\s*=\s*"true"', content)
        if m:
            findings.append(_finding(
                "HIGH", "`usesCleartextTraffic=\"true\"` — the app will talk plain HTTP to any host",
                _line_of(content, m.start()),
        ...
        if m:
            findings.append(_finding(
                "CRITICAL", "`android:debuggable=\"true\"` in the manifest — a release build with this is fully inspectable",
                _line_of(content, m.start()), "Remove it; let the build type control debuggability.",
            ))
        m = re.search(r'android:allowBackup\s*=\s*"true"', content)
        if m:
            findings.append(_finding(
                "MEDIUM", "`allowBackup=\"true\"` — app data, including tokens in SharedPreferences, can be pulled with `adb backup`",
                _line_of(content, m.start()), "Set `allowBackup=\"false\"` unless you ship a backup rules file that excludes secrets.",
            ))
        ...
        for m in re.finditer(r"<(service|receiver|provider)\b([^>]*)>", content):
            attrs = m.group(2)
            if 'android:exported="true"' in attrs and "android:permission=" not in attrs:
                name = re.search(r'android:name="([^"]+)"', attrs)
                findings.append(_finding(
                    "MEDIUM", f"Exported {m.group(1)} `{name.group(1) if name else '?'}` has no `android:permission` — any app can invoke it",
                    _line_of(content, m.start()), "Set `android:exported=\"false\"` or gate it with a signature-level permission.",
                ))
        m = re.search(r'<uses-permission\s+android:name="android\.permission\.SYSTEM_ALERT_WINDOW"', content)
        if m:
            findings.append(_finding(
                "LOW", "Requests SYSTEM_ALERT_WINDOW (draw over other apps) — a review-flag permission most apps do not need",
                _line_of(content, m.start()), "If no feature draws overlays, remove it (often pulled in transitively by a library; `tools:node=\"remove\"` drops it).",
            ))
        return {"findings": findings, "total_issues": len(findings)}
    ...
    def _audit_ios_plist(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        if m:
            findings.append(_finding(
                "HIGH", "App Transport Security is disabled (`NSAllowsArbitraryLoads` true) — plain HTTP to any host",
                _line_of(content, m.start()), "Remove it; add per-domain `NSExceptionDomains` for the one host that genuinely needs it.",
            ))
        m = re.search(r"<key>NSAllowsArbitraryLoadsInWebContent</key>\s*<true\s*/>", content)
        if m:
            findings.append(_finding(
                "MEDIUM", "`NSAllowsArbitraryLoadsInWebContent` true — web views may load over HTTP",
                _line_of(content, m.start()), "Remove unless a web view must load a specific HTTP origin.",
            ))
        return {"findings": findings, "total_issues": len(findings)}
    ...
    def _audit_wrangler(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        if vars_block:
            for m in re.finditer(r"^\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*\"([^\"]*)\"", vars_block.group(1), re.M):
                name, value = m.group(1), m.group(2)
                if _SECRET_NAME.search(name) and value and not _PLACEHOLDER.search(value):
                    findings.append(_finding(
                        "CRITICAL", f"`{name}` is committed under `[vars]` — that is plaintext in git and visible in the dashboard",
    ...
    _ROUTE_PATTERNS = (
        r"[\"'`]{path}[\"'`]",                       # any literal of the path
    )
    ...
    def _audit_railway_config(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        hc_path = m.group(1)
        if hc_path == "/":
            return {"findings": findings, "total_issues": 0}   # root nearly always serves
        ...
        for dirpath, dirnames, filenames in os.walk(service_root):
            dirnames[:] = [d for d in dirnames if d not in {"node_modules", ".git", "dist", "build", ".venv", "venv", "__pycache__", ".next"}]
            for fn in filenames:
                if not fn.endswith((".py", ".ts", ".js", ".mjs", ".tsx", ".go", ".rb", ".rs", ".java", ".kt")):
                    continue
                try:
        ...
        if not found:
            findings.append(_finding(
                "HIGH", f"`healthcheckPath` is `{hc_path}` but no source file under this service mentions that path",
                _line_of(content, m.start()),
    ...
    _ENV_IGNORE = {
        "NODE_ENV", "PORT", "HOSTNAME", "CI", "HOME", "PATH", "PWD", "RAILWAY_ENVIRONMENT",
        "RAILWAY_PUBLIC_DOMAIN", "RAILWAY_PRIVATE_DOMAIN", "RAILWAY_SERVICE_NAME", "TZ",
        "PYTHONPATH", "PYTHONUNBUFFERED", "EXPO_PUBLIC_", "VERCEL", "VERCEL_URL", "SHELL",
        "USER", "LANG", "TERM", "DEBUG", "LOG_LEVEL", "JEST_WORKER_ID",
        "NODE_OPTIONS", "GITHUB_ACTIONS", "RAILWAY_GIT_COMMIT_SHA",
    }
    ...
    def _audit_env_example(self, content: str, path: str = "") -> Dict[str, Any]:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            ...
            if re.match(r"(appl|goog)_[A-Za-z0-9]+$", value):
                continue
            if _SECRET_NAME.search(name) and _REAL_LOOKING_VALUE.match(value) and not _PLACEHOLDER.search(value):
                findings.append(_finding(
                    "HIGH", f"`{name}` in the example file has a real-looking value — .env.example is committed and public on a public repo",
                    i, "Replace with a placeholder (`your-key-here`) and rotate the value if it was ever live.",
                ))
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            dirnames[:] = [d for d in dirnames if d not in {"node_modules", ".git", "dist", "build", ".venv", "venv", "__pycache__", ".next", ".expo", "coverage"}]
            for fn in filenames:
                ...
                if re.search(r"(\.test\.|\.spec\.|_test\.|^test_)", fn) or \
                        os.path.basename(dirpath) in {"__tests__", "tests", "test", "__mocks__"}:
                    continue
        ...
        if missing:
            ...
            findings.append(_finding(
                "MEDIUM", f"{len(missing)} env var(s) the code reads are not in .env.example: {shown}",
                None,
