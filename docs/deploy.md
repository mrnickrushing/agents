...
| `GITHUB_OAUTH_CLIENT_ID` / `GITHUB_OAUTH_CLIENT_SECRET` | **Sign in with GitHub** for the *Run agents* panel. Either a classic OAuth App (Settings → Developer settings → OAuth Apps) or a **GitHub App** works; callback URLs `https://agents.rushingtechnologies.com/auth/callback` and `https://agents-server-production-5f19.up.railway.app/auth/callback` (one per origin in `PUBLIC_URL`). A GitHub App (client id starts with `Iv`) issues 8-hour user tokens with refresh tokens — the service renews them automatically — and only sees repositories it is **installed on**: give it *Contents: Read-only* and install it on your account, or the repository dropdown stays empty. The signed-in person's token lists their repositories and clones private ones — no `GITHUB_TOKEN` needed for that. |
| `PUBLIC_URL` | Comma-separated list of the origins the site is served on (`https://agents.rushingtechnologies.com,https://agents-server-production-5f19.up.railway.app`). Each one is a full front door — nothing redirects. The GitHub callback is built from the host the visitor is on when that host is listed, so **every listed origin's `/auth/callback` must be a callback URL on the GitHub App** (GitHub Apps accept several; a classic OAuth App accepts only one). A visit on an unlisted host uses the first origin's callback and lands there after sign-in. |
