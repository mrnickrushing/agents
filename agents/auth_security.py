...
"""
        ...
        return [
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The token issuance/refresh/verify code to review",
                        },
                        "language": {
                            "type": "string",
                            "enum": ["node", "python"],
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The Apple Sign-In handling code (client or server side)",
                        },
                    },
                    "required": ["code"],
                },
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The OAuth flow code to review",
                        },
                        "provider": {
                            "type": "string",
                            "enum": ["google", "generic"],
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The middleware/handler code that checks the shared secret",
                        },
                    },
                    "required": ["code"],
                },
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "code": {
                            "type": "string",
                            "description": "The biometric auth integration code",
                        },
                    },
                    "required": ["code"],
                },
    ...
    def _review_refresh_token_rotation(
        self, code: str, language: str = "node"
    ) -> Dict[str, Any]:
        ...
        is_pure_client_consumer = not re.search(
            r"jwt\.sign\(|create.*refresh.*token|jose\.sign", code, re.IGNORECASE
        ) and re.search(
            r"securestore|keychain|asyncstorage|savetoken|accepttokens",
            code,
            re.IGNORECASE,
        )
        ...
        if has_refresh and not re.search(
            r"rotat|revoke|blacklist|reuse|invalidat|session_?id|family",
            code,
            re.IGNORECASE,
        ):
            findings.append(
        ...
        is_client_storage = bool(
            re.search(r"securestore|keychain|asyncstorage", code, re.IGNORECASE)
        )
        ...
        token_factory_re = re.compile(
            r"(create|generate|issue|make)\w*refresh\w*token", re.IGNORECASE
        )
        persist_lines = [
            ln
        ...
        if (
            has_refresh
            and not is_client_storage
            and persist_lines
            and not re.search(
                r"hash|sha256|bcrypt|digest", "\n".join(persist_lines), re.IGNORECASE
            )
        ):
            findings.append(
                {
        ...
        if re.search(r"jwt\.verify\s*\(", code) and not re.search(
            r"algorithms\s*:", code
        ):
            findings.append(
                {
        ...
        if re.search(r"jwt\.decode\s*\(", code) and not re.search(
            r"algorithms\s*=", code
        ):
            findings.append(
                {
    ...
    def _review_apple_sign_in(self, code: str) -> Dict[str, Any]:
        ...
        if "nonce" not in code_lower:
            findings.append(
                {
                    "severity": "HIGH",
                    "issue": "No nonce handling found — vulnerable to identity token replay",
                    "fix": "Generate a random nonce client-side, SHA256-hash it for the Apple request, and verify the raw nonce server-side against the token's nonce claim",
                }
            )
        ...
        is_client_token_acquisition = re.search(
            r"signinasync\s*\(", code_lower.replace(" ", "")
        ) is not None and not re.search(
            r"jwt\.(decode|verify)\(|jwtverify\(|createremotejwkset",
            code,
            re.IGNORECASE,
        )
        ...
        uses_jwks = bool(
            re.search(r"jwks|pyjwkclient|get_signing_key", code, re.IGNORECASE)
        )
        unverified_decode = bool(
            re.search(
        ...
        if unverified_decode:
            findings.append(
                {
                    "severity": "CRITICAL",
                    "issue": "Identity token decoded with signature verification explicitly disabled — anyone can forge a token claiming to be any user",
                    "fix": "Fetch Apple's public key via JWKS and verify the signature; never decode with verify_signature=False in a code path that trusts the result",
                }
            )
        elif not uses_jwks:
            findings.append(
                {
                    "severity": "HIGH",
                    "issue": "No JWKS-based signature verification found — the identity token's signature may not actually be checked against Apple's keys",
                    "fix": "Use a JWKS client (e.g. PyJWKClient(APPLE_JWKS_URL) in Python, jwks-rsa in Node) to fetch the signing key and verify the token",
                }
            )
        ...
        if not re.search(r"appleid\.apple\.com", code, re.IGNORECASE):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No issuer check against https://appleid.apple.com found",
                    "fix": "Verify the decoded token's iss claim equals https://appleid.apple.com",
                }
            )
        ...
        if not re.search(r"\baud\b|audience\s*[:=]", code, re.IGNORECASE):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No audience (aud) claim check found",
                    "fix": "Verify the decoded token's aud claim equals your app's bundle identifier",
                }
            )
    ...
    def _review_oauth_flow(self, code: str, provider: str = "google") -> Dict[str, Any]:
        ...
        is_redirect_flow = bool(
            re.search(
                r"redirect_uri|authorization_code|/authorize\b|code_verifier|pkce",
                code,
                re.IGNORECASE,
            )
        )
        if is_redirect_flow and "state" not in code_lower:
            findings.append(
                {
                    "severity": "HIGH",
                    "issue": "No 'state' parameter found in the OAuth redirect flow — vulnerable to CSRF on the callback (an attacker can bind their own account to a victim's session)",
                    "fix": "Generate a random state value, store it (session/cookie), send it in the auth request, and verify it matches on callback",
                }
            )
        ...
        client_side_re = re.compile(
            r"(?<![A-Za-z0-9_])(expo|react-native|mobile|frontend|client-side)(?![A-Za-z0-9_])",
            re.IGNORECASE,
        )
        ...
        if any(client_side_re.search(ln) for ln in secret_lines):
            findings.append(
                {
                    "severity": "CRITICAL",
                    "issue": "client_secret referenced on a line that also looks client-side/mobile — OAuth client secrets must never ship in an app bundle",
                    "fix": "Exchange the authorization code for tokens on your backend only; the mobile app should never see client_secret",
                }
            )
        ...
        if not re.search(r"\baud\b|audience|client_id", code, re.IGNORECASE):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No audience/client_id validation found on the returned id_token",
                    "fix": "Verify the id_token's aud claim matches your OAuth client ID before trusting it",
                }
            )

        if "redirect_uri" in code_lower and not re.search(
            r"allowlist|allowed_redirect|redirect_uris\s*=", code, re.IGNORECASE
        ):
            findings.append(
                {
                    "severity": "INFO",
                    "issue": "redirect_uri usage found — confirm it's validated against a server-side allowlist, not accepted from client input",
                    "fix": "Validate redirect_uri against a fixed allowlist rather than trusting whatever the client sends",
                }
            )
    ...
    def _audit_shared_secret_auth(self, code: str) -> Dict[str, Any]:
        ...
        secret_re = re.compile(
            r"(x-api-key|internal_api_key|app[_-]?secret|admin[_-]?secret)",
            re.IGNORECASE,
        )
        ...
        if secret_re.search(code):
            ...
            if insecure_compare and not re.search(
                r"timingsafeequal|compare_digest|constant.?time", code, re.IGNORECASE
            ):
                findings.append(
                    {
                        "severity": "MEDIUM",
        ...
        looks_like_user_data_route = bool(
            re.search(
                r"router\.|app\.(?:get|post|put|patch|delete)|@router\.|current_user|user[_-]?id",
                code,
                re.IGNORECASE,
            )
        )
        if (
            looks_like_user_data_route
            and re.search(r"(x-api-key|internal_api_key)", code, re.IGNORECASE)
            and not re.search(
                r"jwt|bearer|session|req\.user|current_user", code, re.IGNORECASE
            )
        ):
            findings.append(
                {
    ...
    def _review_biometric_auth(self, code: str) -> Dict[str, Any]:
        ...
        if not re.search(r"isenrolled|isavailable|hardware", code, re.IGNORECASE):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No isEnrolled/isAvailable/hasHardware check before offering biometric auth",
                    "fix": "Check LocalAuthentication.hasHardwareAsync() and isEnrolledAsync() before showing the Face ID option",
                }
            )
        ...
        if not re.search(r"fallback|passcode|password", code_lower):
            findings.append(
                {
                    "severity": "MEDIUM",
                    "issue": "No fallback path for devices without biometrics enrolled",
                    "fix": "Offer device passcode or password as a fallback when biometrics aren't available/enrolled",
                }
            )

        if re.search(r"authenticateasync|biometric", code_lower) and not re.search(
            r"keychain|securestore|token|credential", code_lower
        ):
            findings.append(
                {
                    "severity": "HIGH",
